...
router.post("/flightbooking/:id", async (req: Request, res: any, next: NextFunction) => {
    try {
        const id = req.params.id;
        const {email,mobile, flight_id, no_of_seats, booking_date, booking } = req.body;

        // Check if the booking array is valid and matches the number of seats
        if (!Array.isArray(booking) || booking.length !== no_of_seats) {
            return res.status(400).json({ message: ["Invalid booking data: mismatch in number of seats and passengers."] });
        }

        // Fetch the flight to check if it exists and get the price
        const flight = await Flight.findById(flight_id);
        if (!flight) {
            return res.status(404).json({ message: ["Flight not found"] });
        }

        // Check for existing bookings to determine the highest used seat number
        const existingBookings = await Booking.find({ flight_id }) as any;

        // Extract used seat numbers and find the highest used seat number
        const usedSeatNumbers = existingBookings.flatMap((booking:any) =>
            booking.booking.map((passenger:any) => passenger.seat_no)
        );

        // Find the next available seat number
        const highestSeatNumber = Math.max(
            ...usedSeatNumbers.map((seat:any) => parseInt(seat.split('-')[1], 10) || 0),
            0 
        );
        const nextAvailableSeatNumber = highestSeatNumber + 1;

        // Generate seat numbers dynamically for each passenger in the booking array
        const generatedBooking = booking.map((passenger, index) => {
            const seatNo = `seat:${flight.flight_no}-${String(nextAvailableSeatNumber + index).padStart(3, '0')}`; // Format seat number
            return {
                ...passenger,
                seat_no: seatNo // Assign generated seat number
            };
        });

        // Check for existing bookings with the same flight_id and generated seat numbers
        const seatNumbers = generatedBooking.map(passenger => passenger.seat_no);
        const conflictBookings = await Booking.find({ flight_id, "booking.seat_no": { $in: seatNumbers } });

        if (conflictBookings.length > 0) {
            return res.status(400).json({ message: ["One or more seats are already booked"] });
        }

        // Calculate the total price based on the flight price and number of seats
        const totalPrice = flight.price * no_of_seats;
        ...
        const newBooking = new Booking({
            ...
            no_of_seats,
            booking: generatedBooking, // This will hold the array of passenger details with generated seat numbers
            price: totalPrice,
            status: "booked"
        });

        // Save the booking to the database
     const newbooking =   await newBooking.save();
     ...
     if(newbooking){
        ...
        await Flight.findByIdAndUpdate(flight_id, { $inc: { seat_capacity: -no_of_seats } });
...
router.delete("/canclebooking/:id", async (req: Request, res: any, next: NextFunction) => {
    try {
        ...
        if (!booking) {
            ...
            // Get the number of seats booked and the flight id
            const flightId = booking.flight_id;
            const noOfSeats = booking.no_of_seats;

            // Delete the booking
            const deletedBooking = await Booking.findByIdAndDelete(id);
            if (deletedBooking) {
                // Increment the seat capacity in the flight document
                await Flight.findByIdAndUpdate(flightId, { $inc: { seat_capacity: noOfSeats } });

                // Optionally, if you want to log or perform additional actions with the deleted booking data
